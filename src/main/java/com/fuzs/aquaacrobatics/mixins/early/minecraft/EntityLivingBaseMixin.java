...
//            return false; /* pretend not to be in water */
    ...
    private int getNewAirValue(int original) {
        if (ConfigHandler.MiscellaneousConfig.slowAirReplenish && original == 300 && this.getAir() >= -20 && !aqua$isLosingAir()) {int oldAirValue = Math.max(this.getAir(), 0);
            return Math.min(oldAirValue + 4, 300);
        }
        return original;
    }

    @Redirect(method = "moveEntityWithHeading", at = @At(value = "FIELD", opcode = Opcodes.GETFIELD, target = "net/minecraft/entity/EntityLivingBase.isCollidedHorizontally:Z", ordinal = 1))
    private boolean isJumpingOnLadder(EntityLivingBase instance) {
        if (ConfigHandler.MovementConfig.newClimbingBehavior)
            ...
            return instance.isCollidedHorizontally;
    }
}
